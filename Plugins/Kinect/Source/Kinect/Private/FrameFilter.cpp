...
Pixel Depth Correction is removed as it is handled in Kinect Device
...
Kinect::FrameBuffer& FrameFilter::filterThreadMethod(void)
	{
	...
	// 	{
		...
		{
		// FScopeLock inputLock(&inputFrameMutex);
		//
		...
		for(unsigned int y=0;y<size[1];++y)
			...
			for(unsigned int x=0;x<size[0];++x,++ifPtr,++abPtr,sPtr+=3,++ofPtr,++nofPtr)
				{
				...
				float newCVal=static_cast<float>(newVal);
				...
				if(sPtr[0]>=minNumSamples&&sPtr[2]*sPtr[0]<=maxVariance*sPtr[0]*sPtr[0]+sPtr[1]*sPtr[1])
					...
					if(FMath::Abs(newFiltered-*ofPtr)>=hysteresis)
						{
...
void FrameFilter::setValidElevationInterval(double newMinElevation,double newMaxElevation)
	{
	...
	maxPlane[1]=0.0f;
	maxPlane[2]=1.0f;
	maxPlane[3]=-float(newMaxElevation)+0.5f;
	minPlane[0]=0.0f;
	minPlane[1]=0.0f;
	minPlane[2]=1.0f;
	minPlane[3]=-float(newMinElevation)-0.5f;
