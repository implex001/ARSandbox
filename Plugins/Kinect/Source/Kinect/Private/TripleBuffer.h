...
Adapted to Unreal Engine
Copyright (c) 2023 Joseph Goh
...
class TripleBuffer
	...
	static const UINT8 writtenMask=0x80U; // Bit indicating the "written" flag
	static const UINT8 lockedShift=4U; // LSB position of buffer index currently locked for read access
	static const UINT8 lockedMask=0x30U; // Bits containing the locked buffer index
	static const UINT8 mostRecentShift=2U; // LSB position of buffer index most recently written to
	static const UINT8 mostRecentMask=0x0cU; // Bits containing the most recent buffer index
	static const UINT8 availableShift=0U; // LSB position of buffer index currently locked for write access
	static const UINT8 availableMask=0x03U; // Bits containing the available buffer index
	...
	Value& startNewValue(void) // Prepares buffer to receive a new value
		...
		return buffer[(bufferState&availableMask)>>availableShift];
		}
	void postNewValue(void) // Marks a new buffer value as most recent after data has been written
		...
		while(true)
			...
			UINT8 newBs=writtenMask|(bs&lockedMask)|((bs&mostRecentMask)>>2)|((bs&availableMask)<<2);
			
			/* Try writing the new buffer state to shared memory and bail out if it succeeded: */
			if((newBs=compareAndSwap(bs,newBs))==bs)
				break;
	...
	bool hasNewValue(void) const // Returns true if a new data value is available for the consumer
		...
		return (bufferState&writtenMask)!=0x00U;
		}
	bool lockNewValue(void) // Locks the most recently written value; returns true if the value is new
		...
		while(true)
			...
			UINT8 newBs=((bs&lockedMask)>>2)|((bs&mostRecentMask)<<2)|(bs&availableMask);
	...
	const Value& getLockedValue(void) const // Returns the currently locked value
		...
		return buffer[(bufferState&lockedMask)>>lockedShift];
		}
	...
	UINT8 compareAndSwap(UINT8 testValue,UINT8 newValue)
	{
		UINT8 result=bufferState;
		if(bufferState==testValue)
			bufferState=newValue;
		return result;
	}
